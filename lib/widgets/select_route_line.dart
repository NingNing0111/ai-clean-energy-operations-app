...
class SelectRouteLine extends StatelessWidget {
  final void Function(LabelVO<String>?)? onChanged;

  const SelectRouteLine({super.key, this.onChanged});

  @override
  Widget build(BuildContext context) {
    return FutureBuilder<List<LabelVO<String>>>(
      future: _loadRouteLine(),
      builder: (context, snapshot) {
        return DropdownSearch<LabelVO<String>>(
          ...
          decoratorProps: DropDownDecoratorProps(
            decoration: InputDecoration(labelText: "收运线路", hintText: "请选择收运线路"),
          ),
  ...
  // 异步加载车辆数据方法
  Future<List<LabelVO<String>>> _loadRouteLine() async {
    try {
      final res = await simpleRouteLine();
      if (res.code == 0) {
        return res.data;
      } else {
        return [];
